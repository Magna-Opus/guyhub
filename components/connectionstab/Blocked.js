...
import BlockedList from '../connectioncomponent/BlockedList';
...
class Following extends Component {
    ...
    followingList=()=>{
        GetWithToken('blocked',this.state).then((result)=>{
            console.log("Users blocked",result)
            if(result.data.status===1)
            this.setState({items:result.data.blockuser})
            else
            {
                this.setState({items:result.data.blockuser})

            this.setState({notfound:'Not Blocked Anyone'})
            }
           })
    }
        ...
        return (
            <View>
                ...
                (items)?items.map((item,index)=>{
                    ...
                    return(
                        <BlockedList getfollowinglist={this.getUserData.bind(this)} key={item.ID} imageurl={imageurl} userid={item.ID} name={item.display_name}/>
                    )
