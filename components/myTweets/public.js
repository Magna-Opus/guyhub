...
class PublicTweets extends Component {
    ...
    getPublicTweets=async()=>{
        ...
        this.setState({authtoken:userInfo,post_type:'public'},()=>{
          ...
          PostWithToken('gettweet/my', this.state).then((result)=>{   
              console.log(result)         
            ...
            <View style={styles.container}>
                ...
                this.state.users?this.state.users.map((user,index)=>{
                            ...
                            return(
                                        ...
                                        <PublicList matcheduser={this.getPublicTweets.bind(this)} key={user.ID} id={user.ID} name={user.user.display_name} imageurl={imageurl} date={user.tweet_date} time={user.tweet_time} tweet={user.content} dislikes={(user.unlikes[0])?user.unlikes[0].unlikes:0} like={(user.likes[0])?user.likes[0].likes:0} userid={user.userid} shareurl={user.tweet_url}/>
                            )
